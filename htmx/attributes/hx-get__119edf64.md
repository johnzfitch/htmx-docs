...
- Source URL: https://htmx.org/attributes/hx-get/
- Source file: `fetch/htmx_org_119edf64.html`
- Hash: `119edf64`
...
This example will cause the `button` to issue a `GET` to `/example` and swap the returned HTML into the `innerHTML` of the `button`.

### Notes

- `hx-get` is not inherited
- By default `hx-get` usually does not include any parameters. You can use the [hx-params](https://htmx.org/attributes/hx-params/) attribute to change this
