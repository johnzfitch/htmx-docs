...
- Source URL: https://htmx.org/extensions/preload/
- Source file: `fetch/htmx_org_b444d45e.html`
- Hash: `b444d45e`
...
While the CDN approach is simple, you may want to consider <a href="https://blog.wesleyac.com/posts/why-not-javascript-cdn" rel="noopener" target="_blank">not using CDNs in production</a>. The next easiest way to install `preload` is to simply copy it into your project. Download the extension from `https://cdn.jsdelivr.net/npm/htmx-ext-preload`, add it to the appropriate directory in your project and include it where necessary with a `<script>` tag.

For npm-style build systems, you can install `preload` via <a href="https://www.npmjs.com/" rel="noopener" target="_blank">npm</a>:
...
Typical users hover over links for several hundred milliseconds before they click, which gives your server even more time to respond to the request than the `mousedown` option above. <a href="http://instantclick.io/click-test" rel="noopener" target="_blank">Test your own hover timing here.</a>. However, be careful when using this option because it can increase server load by requesting resources unnecessarily.
...
The behavior for this plugin was inspired by the work done by <a href="https://github.com/dieulot" rel="noopener" target="_blank">Alexandre Dieulot</a> on <a href="http://instantclick.io/" rel="noopener" target="_blank">InstantClick</a>, which is released under the MIT license.
