# hx-redirect__75efc359

- Source URL: https://htmx.org/headers/hx-redirect/
- Source file: `fetch/htmx_org_75efc359.html`
- Hash: `75efc359`
...
This response header can be used to trigger a client side redirection to a new url that will do a full reload of the whole page. It uses the browser to redirect to the new location which can be useful when redirecting to non htmx endpoints that may contain different HTML `head` content or scripts. See [`HX-Location`](https://htmx.org/headers/hx-location/) if you want more control over the redirect or want to use ajax requests instead of full browser reloads.
